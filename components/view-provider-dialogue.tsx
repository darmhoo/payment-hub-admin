...
function ViewProviderDialog({ provider, open, onOpenChange }: ViewProviderDialogProps) {
  ...
  const details = [
    ...
    },
    {
      ...
      valueClassName: provider.Active ? 'text-emerald-600' : 'text-red-600',
    },
    {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>View provider configuration and settings.</DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6">
          ...
          <section>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              {details.map(({ label, value, className, valueClassName }) => (
                <DetailItem
                  key={label}
                  label={label}
                ...
                {provider.Settings.map((setting) => (
                  <div key={setting.ID} className="rounded-lg border bg-muted/20 p-3">
                    <div className="flex items-start justify-between gap-4">
                      <span className="break-all text-sm font-medium">{setting.Key}</span>
                      ...
                      >
                        {setting.Encrypted ? 'Encrypted' : 'Plain text'}
                      </span>
                    </div>
