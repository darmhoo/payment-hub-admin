    ...
    async function fetchProvider() {
      try {
        ...
        const selectedProvider = providerList.find((item: any) => {
          return String(item?.ID ?? item?.id ?? item?.provider_id ?? '') === String(id);
        });
              ...
              <Table>
                ...
                <TableBody>
                  <TableRow>
                    ...
                    <TableCell>{provider?.name ?? provider?.Name ?? 'N/A'}</TableCell>
                  </TableRow>
                  <TableRow>
                    <TableCell className="font-medium">Driver</TableCell>
                    <TableCell>{provider?.driver ?? provider?.Driver ?? 'N/A'}</TableCell>
                  </TableRow>
                  <TableRow>
                    <TableCell className="font-medium">Category</TableCell>
                    <TableCell>{provider?.category ?? provider?.Category ?? 'N/A'}</TableCell>
                  </TableRow>
                  <TableRow>
                    <TableCell className="font-medium">Environment</TableCell>
                    <TableCell>{provider?.environment ?? provider?.Environment ?? 'N/A'}</TableCell>
                  </TableRow>
                  <TableRow>
                    <TableCell className="font-medium">Priority</TableCell>
                    <TableCell>{provider?.priority ?? provider?.Priority ?? 'N/A'}</TableCell>
                  </TableRow>
                  <TableRow>
                    <TableCell className="font-medium">Status</TableCell>
                    <TableCell>
                      {isActive === undefined ? 'N/A' : isActive ? 'Active' : 'Inactive'}
                    </TableCell>
                  </TableRow>
                  <TableRow>
                          ...
                          <TableCell>
                            <div className="flex items-center gap-2">
                              <span className="font-mono text-sm">
                                {!hasValue ? '—' : isHidden ? '••••••••' : String(row.value)}
                              </span>
                                    ...
                                    <TooltipTrigger
                                      render={
                                        <Button
                                          ...
                                          size="icon"
                                          className="h-6 w-6"
                                          onClick={() => copySettingValue(row.value)}
...
function resolveActive(provider: Record<string, any> | null): boolean | undefined {
...
function resolveDefault(provider: Record<string, any> | null): boolean {
  ...
  return Boolean(provider.is_default ?? provider.IsDefault ?? provider.default ?? false);
}

function extractSettings(provider: Record<string, any> | null): ProviderSetting[] {
  if (!provider) {
    return [];
  }
  ...
  if (Array.isArray(settingSource)) {
    ...
    return settingSource.map((item) => {
      ...
      return {
        ...
        value: getField(normalizedSetting, ['value', 'setting_value', 'settingValue', 'val']) ?? '',
        encrypted: Boolean(
          getField(normalizedSetting, [
            'encrypted',
            'is_encrypted',
  ...
  if (settingSource && typeof settingSource === 'object') {
    return Object.entries(settingSource).map(([key, value]) => {
      ...
      return {
        key,
        value: valueObject
          ? (getField(valueObject, ['value', 'setting_value', 'settingValue']) ?? '')
          : value,
        encrypted: valueObject
          ? Boolean(getField(valueObject, ['encrypted', 'is_encrypted', 'isEncrypted']) ?? false)
          : false,
      };
    });
  }
...
function getField(obj: Record<string, any>, candidates: string[]) {
  const lowerToActualKey = new Map(Object.keys(obj).map((k) => [k.toLowerCase(), k]));
